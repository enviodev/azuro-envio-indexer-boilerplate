...
import {
  deserialiseConditionV2Result,
  getConditionV2FromId,
} from "../contracts/corev2";
...
Corev2Contract_ConditionCreated_handlerAsync(async ({ event, context }) => {
  ...
  const _conditionData = await getConditionV2FromId(
    event.srcAddress,
    event.chainId,
    conditionId,
    context
  ...
  if (!coreContractEntity) {
    throw new Error(
      `v2 ConditionCreated coreContractEntity not found. coreAddress = ${coreAddress}`
    );
  }
  ...
  if (!gameEntity) {
    throw new Error(
      `v2 ConditionCreated gameEntity not found. gameEntityId = ${gameEntityId}`
    );
  }
...
Corev2Contract_ConditionResolved_handlerAsync(async ({ event, context }) => {
  ...
  if (!conditionEntity) {
    throw new Error(
      `v2 handleConditionResolved conditionEntity not found. conditionEntityId = ${conditionEntityId}`
    );
  }
...
Corev2Contract_ConditionStopped_handler(async ({ event, context }) => {
  const conditionId = event.params.conditionId;
  const coreAddress = event.srcAddress;
...
Corev2Contract_OddsChanged_handlerAsync(async ({ event, context }) => {
  ...
  for (let i = 0; i < conditionEntity.outcomesIds.length; i++) {
    ...
    if (!outcomeEntity) {
      throw new Error(
        `v2 handleNewBet handleOddsChanged outcomeEntity not found. outcomeEntityId = ${outcomeEntityId}`
      );
    }
