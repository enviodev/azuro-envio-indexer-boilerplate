...
LPv3Contract_GameCanceled_loader(({ event, context }) => {
  if (event.params.gameId == undefined) {
    console.log("asdcqecasdcasdcasdbweawd");
  }

  context.Game.load(
    getEntityId(event.srcAddress, event.params.gameId.toString()),
    {}
  );
});
LPv3Contract_GameCanceled_handler(({ event, context }) => {
  if (event.params.gameId == undefined) {
    console.log("asdcqasdcasdecbweawd");
  }
...
LPv3Contract_GameShifted_loader(({ event, context }) => {
  if (event.params.gameId == undefined) {
    console.log("asdcqasdcasecbweawd");
  }
  context.Game.load(
    getEntityId(event.srcAddress, event.params.gameId.toString()),
    {}
...
LPv3Contract_LiquidityManagerChanged_handler(({ event, context }) => {
  ...
  updateLiquidityManager(event.srcAddress, newAddress, context);
});
...
LPv3Contract_LiquidityRemoved_handlerAsync(async ({ event, context }) => {
  ...
  const { withdrawAmount } = await getNodeWithdrawAmount(
    event.srcAddress,
    event.chainId,
    event.params.depositId
  );
